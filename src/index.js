...
let server = null; // 将 server 变量移到函数外部
...
const connectWithRetry = () => {
  ...
  mongoose.connect(MONGODB_URI, {
    ...
    .then(() => {
      ...
      if (!server || !server.listening) { // 检查服务器是否已在运行
        ...
        server.on('error', (err) => { // 处理服务器启动错误，如 EADDRINUSE
          if (err.code === 'EADDRINUSE') {
            console.error(`错误：端口 ${PORT} 已被占用。请检查是否有其他应用正在使用此端口，或者应用是否未能正确关闭。`);
            process.exit(1); // 或者执行其他清理操作后退出
          } else {
        ...
        console.log('MongoDB已重新连接，服务器已在运行中。');
      }
    })
    .catch((err) => {
